...
getResponseInfo = (socket,requestLineData,callback)->
	fs.stat (path.join ROOT, requestLineData['path']), (err,stats)->
		...
		if err
			...
			responseEntity['size '] = createErrorHtml(responseEntity['statusCode']).length()
		else if requestLineData.method is 'GET' && Buffer.byteLength(requestLineData.path, 'utf8') > 255
			responseEntity['path'] = null
			responseEntity['status'] = 414
			responseEntity['size '] = createErrorHtml(responseEntity['statusCode']).length()
		else if AUTHORIZED_PATH.test(path.join ROOT,responseEntity["path"])
			if stats.isDirectory()
				responseEntity['path'] = path.join responseEntity["path"],'/'
				responseEntity['statusCode'] = 302
			else if stats.isFile()
				responseEntity['statusCode'] = 200
				responseEntity['size'] = stats['size']
				responseEntity['extension'] = path.extname responseEntity['path'].toLowerCase()
		else
			responseEntity['statusCode'] = 403

		if responseEntity['statusCode'] isnt 200 && responseEntity['statusCode'] isnt 302
			errorHtml = createErrorHtml(responseEntity['statusCode'])
			responseEntity['errorHtml'] = errorHtml.body
			responseEntity['size'] = errorHtml.length()
		responseEntity['readStream'] = createReaderStream socket, responseEntity['path'],responseEntity['statusCode']

		responseHeader = createResponseHeader responseEntity, requestLineData, responseEntity
